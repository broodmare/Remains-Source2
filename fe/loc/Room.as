	...
	public class Room {
		...
		public function Room(nxml:XML) {
			...
			if (xml.options.length()) {
				if (xml.options.@tip.length()) {
					tip = xml.options.@tip;
				}
				
				if (xml.options.@level.length()) {
					lvl = xml.options.@level;
				}
				
				if (xml.options.@back.length()) {
					back = xml.options.@back;
				}
