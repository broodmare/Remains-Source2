package fe.inter {
	...
	public class Ctr {
		...
		private var keyDowns:Vector.<Boolean>;	// Pressed keys
		public var keyNames:Vector.<String>;	// [key names by codes]
		private var mbNames:Array;				// [mouse button names]
		private var keys:Array;					// [objects by key codes]
		public var keyObj:Array;				// [objects in order]
		public var keyIds:Array;				// [objects by action ID]

		/* TODO: This can probably replace most of the boolean Action-Key states
		
		private var keyStates:Dictionary;		// Track currently pressed buttons
		
		// Check if the button bound to a certain action is being pressed
		public function isActionPressed(actionId:String):Boolean {
			return keyStates[actionId] == true;
		}

		// Set the state of a button bound to a certain action
		private function setActionState(actionId:String, state:Boolean):void {
			keyStates[actionId] = state;
		}
		...
		*/
		...
		// ### END OF CURRENTLY PRESSED KEYS ###

		public var keyboardMode:int			= 0;
		
		// Timers(?)
		private const dubleT:int			= 5;		// (Double-tap threshold in frames?)
		private var kR_t:int				= 10;		// Right click double-click timer
		private var kL_t:int				= 10;		// Left click double-click timer
		private var  kD_t:int				= 10;		// Key press double-press timer
		private var  scr_t:int				= 0;		// Scrollwheel cooldown timer
		
		public var active:Boolean			= true;
		
		private var KeyboardA:uint			= Keyboard.A;
		private var KeyboardZ:uint			= Keyboard.Z;
		private var KeyboardW:uint			= Keyboard.W;
		private var KeyboardQ:uint			= Keyboard.Q;
		...
		// Constructor
		public function Ctr(loadObj:Object = null) {
		...
		public function step():void {
			...
			if (scr_t > 0) {
				...
				if (scr_t==0) {
					if (keys["scrd"]) {
						this[keys["scrd"].id] = false;
					}
					
					if (keys["scru"]) {
						this[keys["scru"].id] = false;
					}
				}
			}
		}
		...
		public function gotoDef():void {
			...
			for (var i in keyXML.key) {
				...
				if (keyXML.key[i].@def.length()) {
					obj.a1 = keyXML.key[i].@def.toString();
				}
				
				if (keyXML.key[i].@alt.length()) {
					obj.a2 = keyXML.key[i].@alt.toString();
				}
		...
		public function load(arr):void {
			for (var i in arr) {
				if (keyIds[i]) {
					checkKey(arr[i].a1);
					checkKey(arr[i].a2);
					
					if (keyIds[i].a1 != Keyboard.TAB) {
						keyIds[i].a1 = arr[i].a1;
					}
		...
		// Check if a keybind is already used for something and if so, clear it when rebinding (I think)
		...
		public function permissKey(key:uint):Boolean {
			if (key==Keyboard.CONTROL || key==Keyboard.ESCAPE || key==Keyboard.TAB || key==Keyboard.CAPS_LOCK || key==Keyboard.DELETE ||  key==Keyboard.END ||  key==Keyboard.HOME ||  key==Keyboard.INSERT) {
				return false;
			}
		...
		// [request completed]
		private function requestOk(nkey):void {
			setkeyOn = false;
			setkeyRequest = nkey;
		...
		public function onMouseMove1(event:MouseEvent):void {
			...
			if (World.w.gui) {
				if (event.stageY < 100 && event.stageX > World.w.swfStage.stageWidth - 400) {
					World.w.gui.infoAlpha = 0.20;
				}
				else {
					World.w.gui.infoAlpha = 1.00;
				}
		...
		public function onMouseWheel1(event:MouseEvent):void {
			...
			if (event.delta < 0 && keys["scrd"]) {
				this[keys["scrd"].id] = true;
			}
			
			if (event.delta > 0 && keys["scru"]) {
				this[keys["scru"].id] = true;
			}
		...
		public function onKeyboardDownEvent(event:KeyboardEvent):void {
			if (setkeyOn) {
				if (permissKey(event.keyCode)) {
					requestOk(event.keyCode);
				}
			...
			if (!World.w.consoleActive) {
				if (event.keyCode < 256 && !keyDowns[event.keyCode]) {
					if (keys[event.keyCode]) {
						...
						if (keys[event.keyCode].id == "keyLeft" && kL_t < dubleT) {
							keyDubLeft = true;
						}
						
						if (keys[event.keyCode].id == "keyRight" && kR_t < dubleT) {
							keyDubRight = true;
						}

						if (keys[event.keyCode].id == "keySit" && kD_t < dubleT) {
							keyDubSit = true;
						}
					}
				}
		...
		public function onKeyboardUpEvent(event:KeyboardEvent):void {
			...
			if (keys[event.keyCode]) {
				...
				if (keys[event.keyCode].id == "keyLeft") {
					kL_t = 0;
				}
				
				if (keys[event.keyCode].id == "keyRight") {
					kR_t = 0;
				}
				
				if (keys[event.keyCode].id == "keySit") {
					kD_t = 0;
				}
			}
		}
