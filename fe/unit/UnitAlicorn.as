package fe.unit {
	...
	public class UnitAlicorn extends UnitPon {
		...
		public function UnitAlicorn(cid:String=null, ndif:Number=100, xml:XML=null, loadObj:Object=null) {
			...
			currentWeapon = WeaponManager.reference.cloneWeapon('alilight');
			...
			if (tr==3) {//зелёный
				...
				shitArmor=50;
				shitMaxHp=500;
				psyWeapon = WeaponManager.reference.cloneWeapon('alipsy');
				WeaponManager.reference.setOwner(psyWeapon, this);
		...
		override protected function control():void {
			...
			if (aiSpok>0 || tr==3 && osob || t_shit>150) {
				t_shit--;
			}
			
			if (shithp<=0 && t_shit<=0) {
				castShit();
			}
			...
			if (aiState==2 && t_alarm>=0) {
				...
				if (t_alarm == 0) {
					loc.enemySpawn(false,false,'alicorn');
				}
			}
			
			if (tr == 3) {
				allVulnerMult = (shithp > 0) ? 0.40 : 1;	// [Calculated damage from attacks is halved]
			}
			else {
				allVulnerMult = (shithp > 0) ? 0.60 : 1;	// [Calculated damage from attacks is halved]
			}
			...
			if (aiTCh>0) {
				...
				if (aiSpok==0) {
					...
					if (aiState==0 && superSilaTip==1 && isrnd() && stroll) {
						superSila();	//включить невидимость в спокойном состоянии
					}
			...
			if (World.w.enemyAct>1 && aiTCh%10==1) {
				...
				if (findCel()) {
					...
					if (aiTCh%50==1 && (aiState==2 || aiState==3)) {
						setCel(null, celX+Math.random()*320-160, celY+Math.random()*160-80);
					}
