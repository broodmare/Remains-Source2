package fe.unit {
	...
	public class Mine extends Unit {
		...
		public var explRadius:Number	=   0.00;		//радиус взрыва, если 0, то взрыва нет
		public var wdestroy:Number		=  10.00;		//урон блокам
		public var damage1:Number		=  12.00;		//урон юнитам
		public var otbros1:Number		=   5.00;		//отброс
		...
		public function Mine(cid:String=null, ndif:Number=100, xml:XML=null, loadObj:Object=null) {
			...
			boundingBox.width = node.@sX;
			boundingBox.height = node.@sY;
			
			if (node.snd.@dem.length()) {
				sndDem=node.snd.@dem;
			}

			if (node.snd.@sens.length()) {
				sndSens=node.snd.@sens;
			}
			...
			if (visNode.@tipdec.length()) {
				tipDecal = visNode.@tipdec;
			}
		...
		public override function putLoc(nloc:Location, nx:Number, ny:Number):void {
			...
			if (loc.tipEnemy==2 && fraction==F_RAIDER) {
				fraction=Unit.F_ROBOT;
			}
		}
		...
		public function remine():void {
			...
			if (!loc.train) {
				LootGen.lootId(loc, coordinates.X, coordinates.Y, id);
			}
			
			if (sndDem != "") {
				Snd.ps(sndDem, coordinates.X, coordinates.Y);
			}
			...
			if (aiState==1 && detectionDelay <= 0 && sens>0) { //взведена, поиск целей
				if (aiN%4==0) {
					for each (var un:Unit in loc.units) {
						if (un==null || un.activateTrap==0 || un.activateTrap==1 && fraction!=Unit.F_PLAYER && un.fraction!=Unit.F_PLAYER || !isMeet(un) || un.sost==3 || un.fraction==fraction || un.fraction==0) {
							continue;
						}
				...
				if (sndSens != "" && explTime % 5 == 3) {
					Snd.ps(sndSens, coordinates.X, coordinates.Y);
				}
