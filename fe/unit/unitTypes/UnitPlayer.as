package fe.unit.unitTypes
{
	...
	public class UnitPlayer extends UnitPon
	{
		...
		var freeAnim:int=0;
		var prev_levit;
		public var f_levit:Boolean=false;
		public var f_die:Boolean=false;
		public var f_dash:Boolean=false;
		public var f_stealth:Boolean=false;
		public var f_inv:Boolean=false;
		...
		public function UnitPlayer(cid:String=null, ndif:Number=100)
		{
			...
			brake = 2;
			maxjumpp = 8;
			plavdy = accel * 0.50;	// Swimming
			levidy = accel * 0.25;	// Levitation
			if (World.w.alicorn) levidy = accel * 0.5;
			hp=maxhp;
			reloadbar.y=-objectHeight-10;
			
			weaponKrep = 0;	// [0 - weapon levitation, 1 - hold]
		...
		private function intializePetArray():void
		{
			pets = [ ];

			var tempPet:UnitPet;

			tempPet = new UnitPet('phoenix');
			tempPet.gg = this;
			tempPet.setLevel(pers.level);
			pets['phoenix'] = tempPet;
			defpet = tempPet;
			
			tempPet = new UnitPet('owl');
			tempPet.gg = this;
			tempPet.setLevel(pers.level);
			pets['owl'] = tempPet;
			tempPet.hp = tempPet.maxhp * pers.owlhpProc;
			if (pers.owlhpProc <= 0) tempPet.sost = 4;
			
			tempPet = new UnitPet('moon');
			tempPet.gg = this;
			tempPet.setLevel(pers.level);
			pets['moon'] = tempPet;
